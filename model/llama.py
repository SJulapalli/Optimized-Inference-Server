...
from model.rope_utils import initialize_rope
...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        # Reshape to (B * L, num_heads, 1, head_dim) for RoPE with invidual positions.
        queries = queries.reshape(B * L, self.n_heads, 1, -1)
        keys = keys.reshape(B * L, self.n_kv_heads, 1, -1)

        # Pulling positions from the cache will be done elsewhere for now, but that's not strict.
        queries = self.rope(queries, offset=positions)
